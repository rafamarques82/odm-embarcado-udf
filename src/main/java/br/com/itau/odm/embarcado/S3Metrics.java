...
final class S3Metrics {
    ...
    /**
     * Retorna o número total de regras disparadas (compatibilidade com KafkaMetrics).
     */
    static long getRulesFired() {
        return TOTAL_RULES_FIRED.get();
    }
    
    /**
     * Registra resultado de execução (alias para recordExecution).
     */
    static void recordResult(String rulesetPath, long durationMs, int rulesFired, boolean success) {
        recordExecution(rulesetPath, durationMs, rulesFired, success);
    }
    
    /**
     * Força envio do relatório (alias para flushSummary).
     */
    static void flush() {
        flushSummary();
    }
    ...
    static String getStats() {
        return String.format("S3Metrics[sent=%d, errors=%d, executions=%d, ready=%s]",
                TOTAL_SENT.get(), TOTAL_ERRORS.get(), TOTAL_COUNT.get(), S3_READY);
    }
